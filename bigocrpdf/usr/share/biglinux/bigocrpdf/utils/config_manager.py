...
class ConfigManager:
    ...
    def _migrate_from_legacy(self) -> None:
        ...
        if os.path.exists(LEGACY_PATHS["lang"]):
            try:
                with open(LEGACY_PATHS["lang"], "r", encoding="utf-8") as f:
                    lang = f.read().strip()
                    if lang:
        ...
        if os.path.exists(LEGACY_PATHS["quality"]):
            try:
                with open(LEGACY_PATHS["quality"], "r", encoding="utf-8") as f:
        ...
        if os.path.exists(LEGACY_PATHS["align"]):
            try:
                with open(LEGACY_PATHS["align"], "r", encoding="utf-8") as f:
                    align = f.read().strip()
                    if align:
        ...
        if os.path.exists(LEGACY_PATHS["same_folder"]):
            try:
                with open(LEGACY_PATHS["same_folder"], "r", encoding="utf-8") as f:
        ...
        if os.path.exists(LEGACY_PATHS["savefile"]):
            try:
                with open(LEGACY_PATHS["savefile"], "r", encoding="utf-8") as f:
                    folder = f.read().strip()
