...
from utils.format_utils import format_elapsed_time
from utils.i18n import _
from utils.logger import logger
from utils.progress_state import ProgressState
...
PROGRESS_MINIMAL_CHANGE = (
    0.01  # Update if progress changed by 1% (for incremental updates)
)
...
class TerminalPageManager:
    ...
    def _update_progress_bar_incremental(
        self, progress: float, processed_pages: int, total_pages: int
    ) -> None:
        ...
        if self._progress_state.update_fraction(progress):
            ...
            if total_pages > 0:
                progress_text = (
                    f"{progress_percent}% ({processed_pages}/{total_pages} pages)"
                )
            else:
    ...
    def _update_status_text_incremental(self, progress_data: dict) -> None:
        ...
        if not is_processing or progress >= 1.0:
            ...
            self._show_simple_progress_status_incremental(
                processed_files, total_files, time_str
            )
        else:
