...
class SignalManager:
    ...
    def disconnect(self, name: str) -> bool:
        ...
        try:
            if widget.handler_is_connected(handler_id):
                ...
                self._handlers = [
                    (w, h, s) for w, h, s in self._handlers if not (w == widget and h == handler_id)
                ]
                return True
    ...
    def disconnect_widget(self, widget: GObject.Object) -> int:
        ...
        self._named_handlers = {
            name: (w, h) for name, (w, h) in self._named_handlers.items() if w != widget
        }
    ...
    def disconnect_all(self) -> int:
        ...
        for widget, handler_id, _signal in self._handlers:
            try:
                if widget.handler_is_connected(handler_id):
                    widget.disconnect(handler_id)
                    count += 1
