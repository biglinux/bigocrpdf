...
class BigOcrPdfWindow(Adw.ApplicationWindow):
    ...
    def connect_signal(self, widget, signal: str, callback, *args) -> int:
    ...
    def disconnect_signal(self, widget, signal: str) -> bool:
        """Disconnect a signal if it exists.

        Args:
            widget: The widget to disconnect from
            signal: The signal name
        ...
        try:
            ...
            if hasattr(self.ocr_processor, "ocr_queue") and self.ocr_processor.ocr_queue:
                ...
                if hasattr(queue, "running"):
                    for process in tuple(queue.running):
    ...
    def _on_file_processed(
        ...
        def process_in_main_thread():
            ...
            logger.info(
                f"Stored {len(extracted_text)} characters of extracted text for "
