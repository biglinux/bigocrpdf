...
def setup_sigbus_handler() -> None:
    ...
    if hasattr(signal, "SIGBUS"):
        ...
        def handle_sigbus(_signum: int, _frame: Any) -> None:
...
class OcrProcess:
    ...
    def __init__(self, input_file: str, output_file: str, options: dict[str, Any]) -> None:
...
class OcrQueue:
    ...
    def __init__(self, max_concurrent: int = 1) -> None:
        ...
        self.on_file_complete: Callable[[str, str, str], None] | None = None
    ...
    def _monitor_queue(self) -> None:
        ...
        while True:
            ...
            all_completed = False
            callback_input_file: str | None = None
            callback_output_file: str | None = None
            completed_process: OcrProcess | None = None
            ...
            if (
                ...
                self.on_file_complete(callback_input_file, callback_output_file, extracted_text)
    ...
    def _get_extracted_text(
        self,
        completed_process: OcrProcess | None,
        _input_file: str,
        output_file: str,
    ) -> str:
        ...
        if completed_process:
            ...
            if extracted_text:
                logger.info(f"Found extracted_text in process: {len(extracted_text)} characters")
        ...
        temp_text = self._try_get_text_from_temp_file(output_file)
        if temp_text:
            return temp_text
    ...
    def _try_get_text_from_temp_file(self, output_file: str) -> str | None:
        ...
        if os.path.exists(temp_dir):
            ...
            if os.path.exists(temp_file):
                try:
                    ...
                    logger.info(f"Read {len(extracted_text)} characters from temporary file")
                    return extracted_text
                except Exception as e:
    ...
    def get_progress(self) -> float:
        ...
        with self.lock:
            ...
            total_pages_processed = float(self._completed_pages)
            ...
            if self._completed_files >= self._total_files and not self.running and not self.queue:
