"""Document-level tool controller for PDFEditorWindow."""
...
class EditorToolsController:
    """Own the editor's compress, split, and reverse workflows."""

    def __init__(self, owner: PDFEditorWindow) -> None:
        self._owner = owner

    def compress(self, _action, _param) -> None:
        """Show compress dialog and compress the document."""
        if not self._owner._document:
            return
    ...
    def _do_compress(self, quality: int, dpi: int) -> None:
        ...
        if not apply_changes_to_pdf(self._owner._document, tmp_edited):
            self._owner._show_error(_("Failed to prepare document for compression."))
            _rmfile(tmp_edited)
            return
        ...
        if result.success:
            ...
            source_name = os.path.basename(self._owner._pdf_path or "document.pdf")
            dialog.set_initial_name("compressed_" + source_name)
            dialog.set_initial_folder(Gio.File.new_for_path(self._owner._default_save_dir()))
            ...
            _rmfile(tmp_compressed)
            self._owner._show_error(_("Compression failed: {}").format(result.message))
        ...
        try:
            ...
            if file:
                ...
                if path:
                    ...
                    self._owner._show_info(message)
        except GLib.Error:
            pass
        except (OSError, ValueError) as e:
            self._owner._show_error(_("Failed to save compressed PDF: {}").format(str(e)))
        finally:
            remove_tracked_file(tmp_path)

    def split_pages(self, _action, _param) -> None:
    ...
    def split_size(self, _action, _param) -> None:
    ...
    def _finish_split(self, dialog, result, mode: str, value: float) -> None:
        ...
        document = self._owner._document
        if document is None:
            _rmfile(tmp_path)
            return

        if not apply_changes_to_pdf(document, tmp_path):
            self._owner._show_error(_("Failed to prepare document for splitting."))
            _rmfile(tmp_path)
            return

        from bigocrpdf.services.pdf_operations import split_by_pages, split_by_size

        original_stem = Path(document.path).stem
        try:
            if mode == "pages":
            ...
            self._owner._show_info(
                _("Parts: {parts} · Pages: {pages}").format(
                    parts=result_split.parts,
                    pages=result_split.total_pages,
                )
            )
        except Exception as e:
            self._owner._show_error(_("Split failed: {}").format(str(e)))
        finally:
            _rmfile(tmp_path)

    def reverse(self, _action, _param) -> None:
        """Reverse the page order."""
        if not self._owner._document:
            return

        pages = sorted(self._owner._document.pages, key=lambda page: page.position)
        total = len(pages)
        if total < 2:
            return

        self._owner._push_undo()
        for i, page in enumerate(pages):
            page.position = total - 1 - i

        self._owner._document.mark_modified()
        self._owner._page_grid.refresh()
        self._owner._update_status_bar()
